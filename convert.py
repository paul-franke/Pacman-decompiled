...
# 7. Function pointer array declarations inside functions
text = text.replace('void (*func[])() =', 'Action[] func =')
text = text.replace('void (*func[])(ushort param) =', 'Action<ushort>[] func =')
text = text.replace('void (*func[])(int param) =', 'Action<int>[] func =')
text = text.replace('void (*func[])(SOUND_EFFECT*, byte *) =', 'SoundEffectProc1[] func =')
text = text.replace('byte (*func[])(SOUND_EFFECT*) =', 'SoundEffectProc2[] func =')
text = text.replace('void (*func[])(SOUND_EFFECT *, byte *) =', 'SoundEffectProc1[] func =')
text = text.replace('byte (*func[])(SOUND_EFFECT *) =', 'SoundEffectProc2[] func =')

# 8. Fix array parameter syntax in function signatures like 'byte arr[]' -> 'byte* arr'
text = re.sub(r'(\b(?:byte|ushort|uint|sbyte|short|int|XYPOS|TASK|SOUND_EFFECT|SOUNDREGS))\s+([a-zA-Z0-9_]+)\[\]', r'\1* \2', text)

# 9. Comment out includes
text = re.sub(r'^(#include\s+.*)$', r'// \1', text, flags=re.MULTILINE)

# 10. Comment out tableCall_0020 definition from pacman.c
...
# 13. Fix C condition expressions on integer values for C# boolean conditions
# convert '!a' -> '(a == 0)' for C integer variables or properties where appropriate
# e.g., 'if (!inInterrupt)' -> 'if (!inInterrupt)' (bool stays bool)
# convert 'if (a || b)' where a, b are bytes -> 'if (a != 0 || b != 0)'
...
print('Updated convert.py and generated Pacman.cs cleanly!')
